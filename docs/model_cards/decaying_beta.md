...
| roc_auc | 0.9038 |
| brier | 0.1103 |
| recall@precision>=0.5 | 0.8114 |
| recall@precision>=0.8 | 0.5001 |

## Calibration (test split)
Expected calibration error: 0.1836.

## Computational characteristics
Inference latency: 0.00 us/row (measured, wall-clock).
Parameters/complexity: 0.

## Limitations
Ignores SNR/noise/trend entirely -- relies only on the detection-history columns of the feature vector. Lags abrupt regime changes because its geometric decay has a single fixed time constant (`beta_decay_lambda`).

## Known failure modes
Systematically under-reacts right after a channel's state flips (e.g. an EMERGING signal's activation) until enough fresh evidence accumulates to outweigh the decayed prior.
